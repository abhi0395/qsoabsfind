...
def measure_absorber_properties_double_gaussian(index, wavelength, flux, error, absorber_redshift, bound, use_kernel, d_pix, use_covariance=False):
    ...
    for k in range(size_array):
        ...
        if nmf_resi.size > 0 and not np.all(np.isnan(nmf_resi)):
            ...
            # first perform fitting in observed frame
            zf = 1 + absorber_redshift[k]
            obs_init_cond = [amp_first_nmf, line_first * zf, sigma1 * zf, 0.54 * amp_first_nmf, line_second * zf, sigma2 * zf]
            fitting_param_for_spectrum[k], fitting_param_std_for_spectrum[k], _,_,_,_ = double_curve_fit(
                index, double_gaussian, lam_fit * zf, nmf_resi, error_fit=error_flux, bounds=None, init_cond=obs_init_cond, iter_n=2000)
