...
def measure_absorber_properties_double_gaussian(index, wavelength, flux, error, absorber_redshift, bound, use_kernel, d_pix, num_iter=500, use_covariance=False):
    """
    ...
    # no available redshift, just return arrays of zeros
    if size_array == 0:
        return (
    ...
    for k in range(size_array):
        ...
        z1 = find_z_from_minimum(wavelength, flux, line_centre1, absorber_redshift[k], window=pixel_width)
        z2 = find_z_from_minimum(wavelength, flux, line_centre2, absorber_redshift[k], window=pixel_width)
        ...
        if nmf_resi.size > 0 and not np.all(np.isnan(nmf_resi)):
            ...
            obs_fitting_param_for_spectrum, obs_fitting_param_std_for_spectrum, _, _, _,_ = double_curve_fit(
                index, double_gaussian, lam_fit * (1+absorber_redshift[k]), nmf_resi, error_fit=error_flux, bounds=None, init_cond=obs_init_cond, maxefv=num_iter)
