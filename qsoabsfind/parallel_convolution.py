...
from .absorberfinder import convolution_method_absorber_finder_in_QSO_spectra
...
def parallel_convolution_method_absorber_finder_QSO_spectra(fits_file, spec_indices, absorber='MgII', ker_width_pix=[3, 4, 5, 6, 7, 8], coeff_sigma=2.5, mult_resi=1, d_pix=0.6, pm_pixel=200, sn_line1=3, sn_line2=2, use_covariance=False, n_jobs=1):
    ...
    params_list = [(fits_file, index, absorber, ker_width_pix, coeff_sigma, mult_resi, d_pix, pm_pixel, sn_line1, sn_line2, use_covariance) for index in spec_indices]

    # Run the jobs in parallel
    with Pool(processes=n_jobs) as pool:
        results = pool.map(run_convolution_method_single_spectrum, params_list)
    ...
    for result in results:
        index_spec, z_abs, gauss_fit, gauss_fit_std, ew_1_mean, ew_2_mean, ew_total_mean, ew_1_error, ew_2_error, ew_total_error, z_abs_err, sn_1, sn_2 = result
...
def main():
    ...
    results = parallel_convolution_method_single_spectrum(
